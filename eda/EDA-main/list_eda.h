...
public:
   ...
   void eliminaElemento(int n) { //EJERCICIO 5_11
       auto i = begin();
   ...
   void adelantaSegmento5_13(int n, int pos, int lon, int k) {

       if (pos >= n || k > pos || lon == 0 || k == 0)
           return;

       auto nuevoSitioColocacion = this->fantasma;
       auto iniSeg = nuevoSitioColocacion;
       auto finSeg = nuevoSitioColocacion;

       //buscamos inicio nuevo segmento
       int contador = 0;

       while (contador < pos - k) {
           contador++; nuevoSitioColocacion = nuevoSitioColocacion->sig;
       }

       iniSeg = nuevoSitioColocacion;
       while (contador < pos) {
           contador++; iniSeg = iniSeg->sig;
       }

       finSeg = iniSeg;
       while (contador < pos + lon && contador < n) {
           contador++; finSeg=finSeg->sig;
       }

       //quitamos el segmento de atras
       auto aux = nuevoSitioColocacion->sig;

       nuevoSitioColocacion->sig = iniSeg->sig;
       iniSeg->sig = finSeg->sig;
       finSeg->sig = aux;
   ...
   void escribeLista() {
       ...
       while (i != end()) {
           std::cout << i.act->elem << ' '; i.act = i.act->sig;
       }
